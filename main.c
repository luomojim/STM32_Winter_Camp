...
int main(void)
{
    ...
    while (1)
    {
        ...
        // 优先级7（最高）：超声停车触发（距障碍≤1cm 或 超声无返回值）
        if (ultra_stop)
        {
            Motor_Stop();
        }
        // 优先级1：超声减速触发（5~1cm）+ RED1+RED2同触 + 其他红外未触
        ...
        // 优先级2：超声减速触发（5~1cm）+ RED1单触 + 其他红外未触
        else if (ultra_slow && red1 && !red2 && !red3 && !red4 && !red5 && !red6)
        {
            move_back(3);
            turn_right_90();
        ...
        // 优先级3：超声减速触发（5~1cm）+ RED2单触 + 其他红外未触
        else if (ultra_slow && !red1 && red2 && !red3 && !red4 && !red5 && !red6)
        {
            Motor_Stop();
            Delay_ms(200);
        ...
        // 优先级4：RED1/RED2均未触 + 左侧红外触发（RED3/5任一触）
        else if (!red1 && !red2 && (red3 || red5))
        {
            float left_speed = NORMAL_LEFT_SPEED;
            float right_speed = NORMAL_RIGHT_SPEED;
            
            if (red3 && red5)      left_speed += 10;  // RED3+5双触：左轮加速、右轮正常
            else if (red3)         right_speed += 10; // RED3单触：右轮加速、左轮正常
            else if (red5)         left_speed += 10;  // RED5单触：左轮加速、右轮正常
            
            Motor_Forward(limit_pwm(left_speed), limit_pwm(right_speed));
        }
        // 优先级5：RED1/RED2均未触 + 右侧红外触发（RED4/6任一触）
        else if (!red1 && !red2 && (red4 || red6))
        {
            float left_speed = NORMAL_LEFT_SPEED;
            float right_speed = NORMAL_RIGHT_SPEED;
            
            if (red4 && red6)      right_speed += 10; // RED4+6双触：右轮加速、左轮正常
            else if (red4)         left_speed += 10;  // RED4单触：左轮加速、右轮正常
            else if (red6)         right_speed += 10; // RED6单触：右轮加速、左轮正常
            
            Motor_Forward(limit_pwm(left_speed), limit_pwm(right_speed));
        }
        // 优先级6：超声减速触发（5~1cm）+ 无任何红外触发
        else if (ultra_slow && !red1 && !red2 && !red3 && !red4 && !red5 && !red6)
        {
            // 双电机减速50%，低速直行
